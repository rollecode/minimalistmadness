...
(() => {
  ...
  /**
   * Own ad and Telegram banner, with permanent dismiss
   */
  ...
  if (window.localStorage.getItem('hide_tg_banner') !== '1' && document.getElementById('tg-banner')) {
    const tgBanner = document.querySelector('.telegram-group');
    tgBanner.classList.add('showing');
    window.setTimeout(() => tgBanner.classList.remove('dismissed'), 400);
  }
  ...
  document.addEventListener('click', (e) => {
    ...
    if (e.target.closest && e.target.closest('.hide-forever-tg')) {
      const tgBanner = document.querySelector('.telegram-group');
      tgBanner.classList.add('closing');
      tgBanner.classList.remove('showing');
      window.setTimeout(() => tgBanner.classList.add('dismissed'), 400);
